...
def create_user(user: schemas.User_Base, db: Session = Depends(get_db), username=Depends(auth_handler.auth_wrapper)):
...
@app.delete('/api/dogs/{name}', response_model=schemas.User_Base)
def delete_users_dogs(name: str, db: Session = Depends(get_db)):
    user = crud.delete_users(db, name=name)
    if user:
        return True
    else:
...
def create_item_for_Dogs(
        id_user: int, dogs: schemas.Dogs_Base, db: Session = Depends(get_db), username=Depends(auth_handler.auth_wrapper)):
    return crud.create_user_Dogs(db=db, dogs=dogs, id_user=id_user)
...
@app.get('/api/dogs/{name}', response_model=schemas.Dogs_Base)
